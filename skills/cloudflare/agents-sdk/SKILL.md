...
description: Build AI agents on Cloudflare Workers using the Agents SDK. Load
  ...
  scheduled tasks, MCP servers, chat applications, voice agents, or browser
  automation. Covers Agent class, state management, callable RPC, Workflows,
  durable execution, queues, retries, observability, and React hooks. Biases
  towards retrieval from Cloudflare docs over pre-trained knowledge.
x-source: cloudflare/skills
x-source-path: skills/agents-sdk
